...
def process_image(image_url):
    ...
    # based on common betting slip formats
    bet_info = {
        'text': full_text,
        'success': True
    }
    ...
    bet_info['teams'] = extract_teams(full_text)
    bet_info['odds'] = extract_odds(full_text)
    bet_info['amount'] = extract_amount(full_text)
    bet_info['bet_type'] = identify_bet_type(full_text)
...
def extract_teams(text):
    ...
    # Could use a sports database to match against known team names
    lines = text.split('\n')
    teams = []
